...
%{CreateDialogueInterface PhonLex IDLex LCRules LCDRules}

{Inspect Kim}

declare R = rectype(c0:deptype(lambda(v0 'Ind' male(v0)) [path([x1])]) c1:deptype(lambda(v0 'Ind' man(v0)) [path([x1])]) c2:deptype(lambda(v0 'Ind' donkey(v0)) [path([x0])]) c3:deptype(lambda(v0 'Ind' lambda(v1 'Ind' own(v0 v1))) [path([x1]) path([x0])]) c5:deptype(lambda(v0 'Ind' male(v0)) [path([x1])]) c7:deptype(lambda(v0 'Ind' neuter(v0)) [path([x0])]) c8:deptype(lambda(v0 'Ind' lambda(v1 'Ind' like(v0 v1))) [path([x1]) path([x0])]) x0:'Ind' x1:'Ind')

declare R1 = rectype(c0:deptype(lambda(v0 'Ind' named(v0 'Kim')) [path([x0])]) c1:deptype(lambda(v0 'Ind' run(v0)) [path([x0])]) x0:'Ind')

declare R2 = rectype(x0:'Ind')


declare [Types] = {Module.link ['types.ozf']}

{Inspect {Types.instantiateType R2}}

{Inspect {Types.instantiateTypeInModel R Types.model}}

{Inspect Types.model}

{Inspect {Merge [a b c] [d e] Value.'<'}}
