...
export
   Dudamel Beethoven IndefArt
   Tonsättare Dirigent Är Ok Jaha S_NP_VP NP_N %N_Adj_N
   VP_Cop_NP D_D_S D_S
   Lexicon Rules DRules
define
   
   Lexicon = [Dudamel Beethoven IndefArt
	      Tonsättare Dirigent Är Ok Jaha]
   Rules = [S_NP_VP NP_N
	    VP_Cop_NP D_S]
   DRules = [D_D_S]
   ...
   IndefArt = {ResGram.indefArt Abs.indefArt}.utr
   Tonsättare = {ResGram.tonsättare Abs.composer}.sg.indef
   Dirigent = {ResGram.dirigent Abs.conductor}.sg.indef
   Är = {ResGram.vara Abs.be}.pres
   Ok = {ResGram.ok Abs.ok}
   Jaha = {ResGram.jaha Abs.aha}
   
   S_NP_VP = {ResGram.s_NP_VP Abs.s_NP_VP}
   NP_N = {ResGram.nP_N {UResAbs.applyRule Abs.nP_Det_N Abs.indefArt}}
